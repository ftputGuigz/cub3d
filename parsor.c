...
static void	ft_fillNO_path(t_datas *map_datas, t_flags *flags, char *str)
{
	flags->NO = 1;
	map_datas->NO_path = ft_strdup(str);
}

static void	ft_fillWE_path(t_datas *map_datas, t_flags *flags, char *str)
{
	flags->WE = 1;
	map_datas->WE_path = ft_strdup(str);
}

static void	ft_fillEA_path(t_datas *map_datas, t_flags *flags, char *str)
{
	flags->EA = 1;
	map_datas->EA_path = ft_strdup(str);
}

static void	ft_fillSO_path(t_datas *map_datas, t_flags *flags, char *str)
{
	flags->SO = 1;
	map_datas->SO_path = ft_strdup(str);
}

static void	ft_fillsprite_path(t_datas *map_datas, t_flags *flags, char *str)
{
	flags->sprite = 1;
	map_datas->sprite_path = ft_strdup(str);
}

static int	ft_filltexture(int *mapclearance, char *line, t_datas *map_datas, t_flags *flags)
{
	char **tmp;
	...
	if (tmp[0][0] == 'N' && tmp[0][1] == 'O' && !flags->NO)
		ft_fillNO_path(map_datas, flags, tmp[1]);
	else if (tmp[0][0] == 'W' && tmp[0][1] == 'E' && !flags->WE)
		ft_fillWE_path(map_datas, flags, tmp[1]);
	else if (tmp[0][0] == 'E' && tmp[0][1] == 'A' && !flags->EA)
		ft_fillEA_path(map_datas, flags, tmp[1]);
	else if (tmp[0][0] == 'S' && ((tmp[0][1] == 'O' && !flags->SO) || (!tmp[0][1] && !flags->sprite)))
	{
		if (tmp[0][1] == 'O')
			ft_fillSO_path(map_datas, flags, tmp[1]);
		else
			ft_fillsprite_path(map_datas, flags, tmp[1]);
	}
	else
		return (-1);
	return (0);
...
static int	ft_fillres(int *mapclearance, char *line, t_datas *map_datas, t_flags *flags)
{
	char **tmp;
	static int k = 0;
...
static int ft_fillstruct(char *line, char **line_map, t_datas *map_datas)
{
	...
	if (mapclearance < 8)
	{
		...
		if (!line[i])
			return (0);
		if (line[i] == 'R')
			return (ft_fillres(&mapclearance, line, map_datas, &flags));
		else if (line[i] == 'N' || line[i] == 'S' || line[i] == 'W' || line[i] == 'E')
			return (ft_filltexture(&mapclearance, line, map_datas, &flags));
