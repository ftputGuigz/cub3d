...
int	load_textures(t_datas *map)
{
	...
	map->txt2[0].img = mlx_xpm_file_to_image(map->mlx.ptr, map->spr_path, &map->txt2[0].i, &map->txt2[0].j);
	if (map->txt[0].img == NULL || map->txt[1].img == NULL
		|| map->txt[2].img == NULL || map->txt[3].img == NULL || map->txt2[0].img == NULL)
		return (-1);
	...
	map->txt2[0].addr = mlx_get_data_addr(map->txt2[0].img, &map->txt2[0].bits_per_pixel, &map->txt2[0].line_length, &map->txt[4].endian);
	return (0);
}
