...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   display_north_west.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gpetit <marvin@42.fr>                      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/02 12:34:05 by gpetit            #+#    #+#             */
/*   Updated: 2021/03/02 12:34:09 by gpetit           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	init_triangles(ray);
	while ((int)(ray->trix.xc - 0.000001) > 0 && (int)(ray->trix.yc - 0.000001)
	> 0 && map->map[(int)(ray->trix.yc - 0.000001)][(int)(ray->trix.xc -
	0.000001)] != '1')
	{
		ray->trix.xb--;
		ray->trix.xc = ray->trix.xb;
		ray->trix.yc = ray->trix.yb - tanf(ray->trix.angle) *
		fabsf(ray->trix.xa - ray->trix.xb);
	}
	ray->trix.r = sqrtf(powf(fabsf(ray->trix.xb - ray->trix.xa), 2) +
	powf(fabsf(ray->trix.yb - ray->trix.yc), 2));
	while ((int)(ray->triy.xc - 0.000001) > 0 && (int)(ray->triy.yc - 0.000001)
	> 0 && map->map[(int)(ray->triy.yc - 0.000001)][(int)(ray->triy.xc -
	0.000001)] != '1')
	{
		ray->triy.yb--;
		ray->triy.yc = ray->triy.yb;
